...
void toGoal(){
  ...
  if(limit_time>900){
    ...
    printf("まあよい、また明日出直してくるんじゃな。\n");
  }
  else if(itm[20].has_itm == 1){
  printf("よく来てくれた！トイレのソムリエよ！\n");
  printf("おお！このトイレは私好みだ！私の大好きなギャル物のAVがあるじゃないか！\n");
  printf("君はこの会社にとどまるのは惜しい！T〇T〇に就職するべきだよ！これからもより良いトイレを作ってくれよ！\n");//AVget
  }
  else if(itm[0].has_itm == 1 && itm[3].has_itm == 1 && itm[6].has_itm == 1 && itm[12].has_itm == 1){
  printf("よく来てくれた！トイレのソムリエよ！\n");
  printf("ふむふむ、どうにかトイレを作ることは出来たようじゃな.......\n");
    if(total_point >= 26){
      printf("うーん、このトイレは実に良い！良いぞ！さすがソムリエ君、君をトイレ会長に任命しよう。\n");//hiscore
    }
    else{
      printf("うーん、このトイレは最悪だね。最悪。そんなソムリエ君、君は明日から来なくていいよ。自宅待機で。\n");//lowscore
    }
  }
  else{
    printf("よく来てくれた！トイレのソムリエよ！\n");
    printf("おお、ソムリエ君！必要なアイテムを集めずにここに来てしまうとは何事だ！仕方ないやつだな。\n");
    printf("お前にもう一度機会を与えよう！再びこのようなことが起こらぬことをわしは祈っている！\n");//Badend.集めていない
  }
